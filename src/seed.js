export function seedDatabase(firebase) {
    ...
    firebase.firestore().collection('courses').add({
      ...
      genre: 'programming',
      slug: 'C++',
      videos: [{title: 'Function', link: 'EeVSCtVg4FI'},
    ...
    firebase.firestore().collection('courses').add({
      ...
      videos: [{title: ' Service-oriented Architecture Part 1', link: 'jA4MWmcQtUQ'},
    ...
    firebase.firestore().collection('courses').add({
      ...
      videos: [{title: ' DES Algorithm (Encryption)', link: '-1xgf1TGpcI'},
                {title: 'DES Modes of Operation (ECB, CBC, CFB, OFB, CTR)', link: 'pQwW8DvxZMI'},
                ...
                {title: 'Different between MD4 & MD5 (Hashes and message digests) ', link: 'jA9-PF1fKLE'},
    ...
    firebase.firestore().collection('courses').add({
      ...
      videos: [{title: ' Convert Binary number to Octal', link: 'XKRiolZ6lLM'},
                {title: 'FAQ ', link: 'FaV84Az1qcY'}]
    });
    firebase.firestore().collection('courses').add({
      id: getUUID(),
      ...
      videos: [{title: ' Introduction to Inheritance', link: 'QiDBAAe6Mlg'},
                {title: 'Subclass and super class - Inheritance ', link: 'Y18u6xPNC3Q'},
                {title: 'Why Use XOR ^ in DES', link: 'SYgbTPLPcAQ'},
                {title: 'Super Keyword and constructor chain - Inheritance ', link: 'TpwfFRpk79g'},
                {title: 'instanceof and summary of Inheritance - Inheritance', link: 'dp_JPytCgVM'},]
    });
    ...
    firebase.firestore().collection('courses').add({
      ...
      genre: 'programming',
      slug: 'assembly',
      videos: [{title: ' Experiment 1 Swapping Ax , Bx Values 1', link: 'QNIn1dbiv8A'},
    ...
    firebase.firestore().collection('courses').add({
      ...
      videos: [{title: 'Hermite curve by python', link: 'mzCyTYE10ZI'},
                {title: 'How convert any object to egg model by blender', link: 'MgzSpVN9Byw'},
                {title: 'Add Photo To 3D Model In Panda3D ', link: 'gSfBHUOQCp8'},
                {title: 'Computer Graphics Midterm Exam Solution 2017', link: 'jtT-T9h5QRc'}]
    });
    firebase.firestore().collection('courses').add({
      id: getUUID(),
      ...
      videos: [{title: ' Concurrency Deadlock and Starvation ( part 1 )', link: 'PpVlAQNLb5I'},
                ...
                {title: 'Dining Philosophers Problem : Deadlock solution : using Semaphore', link: 'pU3fjsQtk-U'},
                {title: 'Classification of multiprocessors system', link: 'NBooNJaEcSU'},
                {title: 'Assignment of processes to processors ', link: 'alszuj7JlAE'},
                {title: 'Multiprogramming on individual processors and actual dispatching of a process ', link: 'Q0IbLgvM2nE'},
    ...
    firebase.firestore().collection('courses').add({
      ...
      videos: [{title: ' Introduction to Genetic algorithm Explanation', link: 'GoRetaaS4oE'},
